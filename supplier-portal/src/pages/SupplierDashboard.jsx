...
const SupplierDashboard = ({ globalSearchQuery }) => {
    ...
    const stats = [
        { label: 'Solde Portefeuille', value: `${balance.toLocaleString()} F`, icon: <Banknote className="text-primary" />, color: 'bg-primary/10' },
        ...
        { label: 'Total Commandes', value: filteredOrders.length, icon: <BarChart3 className="text-primary" />, color: 'bg-primary/10' },
    ];

    if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-base-content/30 uppercase tracking-widest animate-pulse">Chargement de votre espace...</div>;
    ...
    return (
        <div className="min-h-screen bg-base-200 p-6 md:p-12 space-y-12">
            ...
            {supplierProfile?.status === 'en_attente' && (
                ...
                >
                    <div className="w-16 h-16 bg-base-100 rounded-2xl flex items-center justify-center text-amber-500 shadow-sm shrink-0">
                        <Clock size={32} />
                    </div>
                    <div className="text-center md:text-left">
                        <h4 className="text-xl font-black tracking-tighter text-base-content">Compte en cours de vérification</h4>
                        <p className="text-sm font-bold text-base-content/50 uppercase tracking-widest text-[10px]">L'admin valide actuellement vos informations. Les ventes sont bloquées temporairement.</p>
                    </div>
                </motion.div>
            )}
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-4xl font-black tracking-tighter text-base-content mb-2">Bienvenue, {user?.firstName}</h1>
                    <p className="text-base-content/50 font-bold uppercase tracking-[0.2em] text-[10px]">Console de Gestion Marchand</p>
                </div>
                <button
                    ...
                    className={`flex items-center gap-3 px-8 py-4 rounded-3xl font-black uppercase tracking-widest text-[10px] transition-all shadow-xl ${supplierProfile?.status === 'active'
                        ? 'bg-primary text-white hover:brightness-90 shadow-primary/20'
                        : 'bg-base-300 text-base-content/40 cursor-not-allowed grayscale shadow-none'
                        }`}
                >
            ...
            {boutiques.length > 0 && (
                <div className="flex items-center gap-4 bg-base-100 p-2 rounded-full border border-base-300 shadow-sm w-fit">
                    <button 
                        onClick={() => setSelectedBoutiqueId('all')}
                        className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${selectedBoutiqueId === 'all' ? 'bg-neutral text-white shadow-lg' : 'text-base-content/40 hover:text-base-content/70'}`}
                    >
                    ...
                    {boutiques.map(b => (
                        <button 
                            ...
                            className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${selectedBoutiqueId === b.id ? 'bg-primary text-white shadow-lg' : 'text-base-content/40 hover:text-base-content/70'}`}
                        >
                            {b.name}
                        </button>
                    ))}
                    <button 
                        ...
                        className="w-10 h-10 rounded-full bg-base-200 text-base-content/40 hover:bg-primary/10 hover:text-primary transition-all flex items-center justify-center border border-dashed border-base-300"
                    >
                        <Plus size={16} />
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {stats.map((stat, idx) => (
                    <motion.div
                        ...
                        key={idx}
                        className="bg-base-100 p-8 rounded-[40px] border border-base-300 shadow-xl shadow-base-300/50 flex items-center gap-6"
                    >
                        ...
                        <div>
                            ...
                            <p className="text-3xl font-black text-base-content tracking-tighter">{stat.value}</p>
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
                {/* Product Status */}
                <div className="bg-base-100 rounded-[40px] border border-base-300 shadow-2xl shadow-base-300/50 p-10 space-y-8">
                    ...
                    <div className="space-y-6">
                        {filteredProducts.slice(0, 5).map((product) => (
                            <div 
                                key={product.id} 
                                onClick={() => navigate('/mes-produits')}
                                className="flex items-center justify-between p-6 rounded-3xl bg-base-200 hover:bg-base-300 transition-colors group cursor-pointer"
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <p className="text-[10px] font-bold text-base-content/40">{(parseFloat(product.supplier_price || product.price * 0.9)).toLocaleString()} F (Gain Net)</p>
                ...
                <div className="bg-neutral rounded-[40px] shadow-2xl shadow-primary p-10 text-white space-y-10 relative overflow-hidden">
