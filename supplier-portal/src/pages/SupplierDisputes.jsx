    ...
    return (
        ...
        >
            <motion.div
                ...
                className="bg-white w-full sm:max-w-lg rounded-t-[2rem] sm:rounded-3xl p-6 sm:p-8 shadow-2xl max-h-[90vh] overflow-y-auto"
            ...
            >
                ...
                <div className="flex items-start justify-between mb-6">
                    <div>
                        <h3 className="text-lg sm:text-xl font-black text-slate-900">Répondre au litige</h3>
                ...
                <textarea
                    ...
                    className="w-full p-4 rounded-2xl ring-1 ring-slate-200 focus:ring-2 focus:ring-indigo-400 outline-none text-sm font-medium text-slate-800 placeholder:text-slate-300 resize-none mb-5"
                />

                <div className="flex gap-3">
                    <button
                        onClick={onClose}
                        className="flex-1 py-3.5 rounded-2xl bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-colors"
                    >
                        Annuler
                    </button>
                    <button
                        ...
                        className="flex-1 py-3.5 rounded-2xl bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                        <Send size={14} />
...
const DisputeCard = ({ dispute, onRespond }) => {
    ...
    return (
        <motion.div
            ...
            className="bg-white rounded-3xl border border-slate-100 shadow-sm p-5 flex flex-col gap-4"
        >
            ...
            {/* Description */}
            {dispute.description && (
                <p className="text-sm text-slate-600 font-medium leading-relaxed bg-slate-50 rounded-2xl p-4 line-clamp-3">
                    {dispute.description}
                </p>
            )}
            ...
            {canRespond && (
                <button
                    ...
                    className="w-full py-3.5 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all flex items-center justify-center gap-2"
                >
...
const SupplierDisputes = () => {
    ...
    return (
        <div className="p-4 sm:p-6 md:p-12 min-h-screen bg-slate-50 space-y-6 md:space-y-10 pb-32 md:pb-12">

            {/* Header */}
            <div>
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tighter text-slate-900 mb-1">
                    Mes Litiges
                </h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    ...
                    <button
                        ...
                        className={`${bg} rounded-2xl sm:rounded-3xl p-4 sm:p-5 text-left transition-all ${filterStatus === key ? 'ring-2 ring-offset-2 ring-slate-300' : ''}`}
                    >
            ...
            <div className="w-full overflow-x-auto no-scrollbar">
                <div className="flex gap-1 bg-white p-1.5 rounded-full ring-1 ring-slate-200 flex-nowrap w-max">
            ...
            {filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 sm:py-24 text-center">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-100 rounded-3xl flex items-center justify-center mb-6">
                        <AlertCircle size={30} className="text-slate-300" />
                    </div>
                    <p className="font-black text-slate-900 text-lg sm:text-xl">Aucun litige</p>
