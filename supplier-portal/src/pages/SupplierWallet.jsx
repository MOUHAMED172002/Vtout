        ...
        <div className="max-w-6xl mx-auto p-6 md:p-12 space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-8">
                    <div className="flex items-center justify-between px-4">
                        <h3 className="text-xl font-black text-base-content tracking-tight">Transactions récentes</h3>
                        <span className="text-[10px] font-black uppercase tracking-widest text-base-content/40">Dernières 20</span>
                    </div>
                            ...
                            const sourceLabel = t.source === 'supplier'
                                ...
                                : t.source === 'deliverer'
                                    ...
                                    : t.type === 'earning'
                                        ...
                                        : 'Transaction';
                            return (
                                <div key={t.id} className="bg-base-100 p-6 rounded-3xl border border-base-200 flex items-center gap-6 shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    <div className="flex-1">
                                        <p className="font-black text-base-content text-sm">{sourceLabel} · {t.description}</p>
                                        <p className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest">{new Date(t.createdAt || t.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}</p>
                                    </div>
                                    <div className={`text-lg font-black ${t.type === 'earning' ? 'text-emerald-500' : 'text-base-content'}`}>
                ...
                <div className="space-y-8">
                    <div className="flex items-center justify-between px-4">
                        <h3 className="text-xl font-black text-base-content tracking-tight">Demandes de retrait</h3>
                        <Clock size={20} className="text-base-content/40" />
                    </div>

                    <div className="space-y-4">
                        {stats.payoutRequests.length > 0 ? stats.payoutRequests.map(p => (
                            <div key={p.id} className="bg-base-100 p-6 rounded-3xl border border-base-200 space-y-4">
                                <div className="flex justify-between items-center">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-base-content/40">{new Date(p.createdAt || p.created_at).toLocaleString()}</span>
                                ...
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className="text-2xl font-black text-base-content">{Number(p.amount).toLocaleString()} F</p>
                                        <p className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest mt-1">Via {p.payment_method.toUpperCase()}</p>
                                    </div>
                                    {p.admin_notes && <p className="text-[9px] font-bold text-base-content/40 max-w-[150px] italic">Note: {p.admin_notes}</p>}
                                </div>
                            </div>
                        )) : (
                            <div className="text-center py-10 bg-base-200 rounded-3xl text-base-content/40 font-bold text-sm">
            ...
            {showPayoutModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-neutral/60 backdrop-blur-sm" onClick={() => setShowPayoutModal(false)} />
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-base-100 w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl relative z-10 space-y-8">
                        <div className="space-y-2">
                            <h3 className="text-3xl font-black text-base-content tracking-tighter">Retrait d'argent</h3>
                            <p className="text-sm font-bold text-base-content/40">Choisissez le montant et le mode de retrait.</p>
                        </div>
                        ...
                        <form onSubmit={handlePayoutRequest} className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <input 
                                    ...
                                    placeholder="Ex: 5000" 
                                />
                                <p className="text-[10px] text-right font-bold text-base-content/40">Solde max: {stats.balance.toLocaleString()} F</p>
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <button type="button" onClick={() => setPayoutMethod('momo')} className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-3 transition-all ${payoutMethod === 'momo' ? 'border-primary bg-primary/5 text-primary' : 'border-base-300 text-base-content/40 hover:border-base-300'}`}>
                                    <ShieldCheck size={24} />
                                    <span className="text-[10px] font-black uppercase tracking-widest">Mobile Money</span>
                                </button>
                                <button type="button" onClick={() => setPayoutMethod('bank')} className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-3 transition-all ${payoutMethod === 'bank' ? 'border-primary bg-primary/5 text-primary' : 'border-base-300 text-base-content/40 hover:border-base-300'}`}>
                                    <Banknote size={24} />
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-2 ml-4 mt-2">
                                    ...
                                    />
                                    <label htmlFor="save_details" className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest cursor-pointer select-none">Enregistrer ces informations</label>
                                </div>
                            </div>
