                        ...
                        >
                            ...
                            <div className="border-b border-slate-100 pb-6">
                                <h3 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                                    <Sparkles size={24} className="text-indigo-500" /> Offres & Promotions
                                </h3>
                                <p className="text-xs text-slate-400 font-medium mt-1">Boostez la visibilité et les ventes de votre produit en configurant des promotions ou des options d'achat groupé.</p>
                            </div>

                            {/* Section 1: Promotions Directes */}
                            <div className="space-y-6">
                                <div className="flex items-center gap-2 px-2">
                                    <div className="w-1.5 h-6 bg-indigo-500 rounded-full" />
                                    <span className="text-xs font-black uppercase tracking-widest text-indigo-900">1. Réductions de Prix</span>
                                </div>

                                <div className="grid grid-cols-1 gap-6">
                                    {/* 0. Réduction Simple Option */}
                                    <div className={`p-8 rounded-[2rem] border transition-all duration-300 ${watch("is_promo") ? 'bg-indigo-50/20 border-indigo-200 shadow-lg' : 'bg-white border-slate-100 hover:border-indigo-100 hover:shadow-md'}`}>
                                        <div className="flex items-start justify-between gap-4">
                                            <label className="flex gap-4 cursor-pointer select-none flex-1">
                                                <input 
                                                    type="checkbox" 
                                                    {...register("is_promo")}
                                                    className="checkbox checkbox-indigo checkbox-md rounded-xl mt-1"
                                                />
                                                <div>
                                                    <h4 className="text-base font-black text-slate-900 flex items-center gap-1.5">
                                                        <Percent size={16} className="text-indigo-500" /> Réduction Simple (Prix Barré)
                                                    </h4>
                                                    <p className="text-xs font-medium text-slate-400 mt-1">Affichez un prix d'origine barré à côté du prix de vente réduit pour inciter à l'achat.</p>
                                                </div>
                                            </label>
                                        </div>

                                        {watch("is_promo") && (
                                            <div className="mt-8 pt-8 border-t border-indigo-100/50 space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                                    <div className="space-y-2">
                                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block px-1">Prix de Vente Original *</label>
                                                    ...
                                                    <div className="space-y-2">
                                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block px-1">Pourcentage de Remise</label>
                                                        <div className="relative">
                                                            <input 
                                                                type="number" 
                                                                placeholder="Ex: 15"
                                                    ...
                                                    <div className="space-y-2">
                                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block px-1">Prix de Vente Réduit *</label>
                                                        <div className="relative">
                                                            <input 
                                                                type="number" 
                                                                placeholder="Ex: 8500"
                                                ...
                                                {/* Dynamic calculations flow card */}
                                                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-4">
                                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Aperçu de la tarification client</span>
                                                    <div className="flex flex-wrap items-center gap-4 sm:gap-6 bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
                                                        {watchedOldPrice && (
                                                            <div className="space-y-1">
                                                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Ancien Prix Public</span>
                                                                <span className="text-sm font-bold text-slate-400 line-through">
                                                                    {computePublicPrice(watchedOldPrice, deliveryTiers).toLocaleString()} F CFA
                                                                </span>
                                                            </div>
                                                        )}
                                                        {watchedOldPrice && <div className="h-8 w-px bg-slate-100 hidden sm:block" />}
                                                        <div className="space-y-1">
                                                            <span className="text-[9px] font-bold text-indigo-500 uppercase tracking-widest block">Nouveau Prix Public</span>
                                                        ...
                                                        <div className="ml-auto flex items-center gap-2">
                                                            {computedPercentage > 0 && (
                                                                <span className="px-3 py-1 bg-emerald-500 text-white rounded-xl text-xs font-black shadow-lg shadow-emerald-500/20">
                                                                    -{computedPercentage}%
                                                                </span>
                                                            )}
                                                    ...
                                                    <p className="text-[10px] text-slate-400 font-medium leading-relaxed">
                                                        * Le prix affiché aux clients inclut automatiquement la simulation des frais de livraison pour offrir une expérience sans frais surprise.
                                                    </p>
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div className={`p-8 rounded-[2rem] border transition-all duration-300 ${watch("is_flash_sale") ? 'bg-rose-50/20 border-rose-200 shadow-lg' : 'bg-white border-slate-100 hover:border-rose-100 hover:shadow-md'}`}>
                                        <div className="flex items-start justify-between gap-4">
                                            <label className="flex gap-4 cursor-pointer select-none flex-1">
                                                <input 
                                                    type="checkbox" 
                                                    {...register("is_flash_sale")}
                                                    className="checkbox checkbox-rose checkbox-md rounded-xl mt-1"
                                                />
                                                <div>
                                                    <h4 className="text-base font-black text-slate-900 flex items-center gap-1.5">
                                                        <Flame size={16} className="text-rose-500 fill-rose-500 animate-pulse" /> Vente Flash
                                                    </h4>
                                                    <p className="text-xs font-medium text-slate-400 mt-1">Proposez une offre ultra-limitée dans le temps pour déclencher des achats compulsifs.</p>
                                                </div>
                                            </label>
                                        </div>

                                        {watch("is_flash_sale") && (
                                            <div className="mt-8 pt-8 border-t border-rose-100/50 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                                                ...
                                                <div className="p-4 bg-rose-50 rounded-2xl border border-rose-100/50 flex gap-3">
                                                    ...
                                                    <p className="text-[10px] text-rose-500 font-bold leading-relaxed">
                                                        Le produit apparaîtra en tête d'affiche dans le carrousel des "Ventes Flash" de la page d'accueil avec un compte à rebours dynamique jusqu'à l'expiration de l'offre.
                                                    </p>
                                                </div>
                                            </div>
                                        )}
                            ...
                            {/* Section 2: Configurations Spéciales */}
                            <div className="space-y-6 pt-6 border-t border-slate-100">
                                <div className="flex items-center gap-2 px-2">
                                    <div className="w-1.5 h-6 bg-blue-500 rounded-full" />
                                    <span className="text-xs font-black uppercase tracking-widest text-blue-900">2. Ventes en Lot & Packs</span>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {/* 2. Volume Discounts Option */}
                                    <div className={`p-8 rounded-[2rem] border transition-all duration-300 flex flex-col ${watch("volume_pricing_enabled") ? 'bg-blue-50/20 border-blue-200 shadow-lg' : 'bg-white border-slate-100 hover:border-blue-100 hover:shadow-md'}`}>
                                        <label className="flex gap-4 cursor-pointer select-none">
                                            <input 
                                                type="checkbox" 
                                                {...register("volume_pricing_enabled")}
                                                className="checkbox checkbox-primary checkbox-md rounded-xl mt-1"
                                            />
                                            <div>
                                                <h4 className="text-base font-black text-slate-900 flex items-center gap-1.5">
                                                    <Percent size={16} className="text-blue-500" /> Remises par Quantité
                                                </h4>
                                                <p className="text-xs font-medium text-slate-400 mt-1">Réduisez le prix unitaire lorsque l'acheteur commande plusieurs pièces.</p>
                                            </div>
                                        </label>

                                        {watch("volume_pricing_enabled") && (
                                            <div className="mt-8 pt-8 border-t border-blue-100/50 space-y-6 flex-1 animate-in fade-in slide-in-from-top-4 duration-300">
                                                <div className="grid grid-cols-2 gap-4">
                                                    {/* Tier 1 */}
                                                    <div className="bg-white p-4 rounded-2xl border border-blue-100 space-y-3">
                                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block text-center">Palier 1</span>
                                                        <div className="flex flex-col gap-2">
                                                            <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2 border border-slate-100">
                                                                <span className="text-[9px] font-bold text-slate-400 mr-2">MIN.</span>
                                                                <input 
                                                                    type="number" 
                                                                    placeholder="3" 
                                                                    {...register("volume_pricing.0.qty")} 
                                                                    className="w-full bg-transparent border-none p-0 text-xs font-black text-slate-800 text-center focus:ring-0 outline-none"
                                                                />
                                                                <span className="text-[9px] font-bold text-slate-400 ml-2">PCS</span>
                                                            </div>
                                                            <div className="flex items-center bg-blue-50/50 rounded-xl px-3 py-2 border border-blue-100/50">
                                                                <span className="text-[9px] font-bold text-blue-500 mr-2">DESC.</span>
                                                                <input 
                                                                    type="number" 
                                                                    placeholder="10" 
                                                                    {...register("volume_pricing.0.discount")} 
                                                                    className="w-full bg-transparent border-none p-0 text-xs font-black text-blue-600 text-center focus:ring-0 outline-none"
                                                                />
                                                                <span className="text-[9px] font-bold text-blue-500 ml-2">%</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {/* Tier 2 */}
                                                    <div className="bg-white p-4 rounded-2xl border border-blue-100 space-y-3">
                                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block text-center">Palier 2</span>
                                                        <div className="flex flex-col gap-2">
                                                            <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2 border border-slate-100">
                                                                <span className="text-[9px] font-bold text-slate-400 mr-2">MIN.</span>
                                                                <input 
                                                ...
                                                <p className="text-[9px] text-blue-400 font-bold italic text-center leading-relaxed">
                                                    Ex: 3+ articles = -10%, 5+ articles = -20% de remise sur chaque unité achetée.
                                                </p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className={`p-8 rounded-[2rem] border transition-all duration-300 flex flex-col ${watch("is_kit") ? 'bg-emerald-50/20 border-emerald-200 shadow-lg' : 'bg-white border-slate-100 hover:border-emerald-100 hover:shadow-md'}`}>
                                        <label className="flex gap-4 cursor-pointer select-none">
                                            <input 
                                                type="checkbox" 
                                                {...register("is_kit")}
                                                className="checkbox checkbox-secondary checkbox-md rounded-xl mt-1"
                                            />
                                            <div>
                                                <h4 className="text-base font-black text-slate-900 flex items-center gap-1.5">
                                                    <Package size={16} className="text-emerald-500" /> Kit / Pack Économique
                                                </h4>
                                                <p className="text-xs font-medium text-slate-400 mt-1">Regroupez cet article avec d'autres produits existants pour former un lot.</p>
                                            </div>
                                        </label>
