...
export default function AddressSelector({ 
  ...
  // Flatten the location data for fast search
  const allLocations = useMemo(() => {
    const list = [];
    decoupageData.forEach((dep) => {
      dep.communes.forEach((com) => {
        com.arrondissements.forEach((arr) => {
          arr.quartiers.forEach((q) => {
            const formatted = `${q.lib_quart}, ${arr.lib_arrond}, ${com.lib_com}`;
            list.push({
              departement_id: dep.id_dep,
              departement_label: dep.lib_dep,
              commune_id: com.id_com,
              commune_label: com.lib_com,
              arrondissement_id: arr.id_arrond,
              arrondissement_label: arr.lib_arrond,
              quartier_id: q.id_quart,
              quartier_label: q.lib_quart,
              formattedAddress: formatted,
