    ...
    return (
        <div className="flex flex-col items-center justify-center py-12 px-4 md:px-0 max-w-4xl mx-auto">
            ...
            <div className="text-center mb-12 space-y-4">
                ...
                <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-base-content">
                    {getConfig('supplier_welcome_page_title', 'Propulsez vos ventes.')}
                </h1>
                <p className="text-base-content/50 font-bold uppercase tracking-widest text-[10px] md:text-xs max-w-lg mx-auto">
            ...
            <div className="w-full max-w-2xl mb-12">
                <div className="flex justify-between items-center relative">
                    <div className="absolute top-1/2 left-0 w-full h-1 bg-base-300 -translate-y-1/2 z-0 rounded-full"></div>
                    <div
                    ...
                    {steps.map((s) => (
                        <div key={s.id} className="relative z-10 flex flex-col items-center gap-3">
                            ...
                            <span className={`text-[10px] font-black uppercase tracking-widest ${step >= s.id ? 'text-primary' : 'text-base-content/40'}`}>
                                {s.label}
                            </span>
                        </div>
                    ))}
            ...
            {/* Form Content */}
            <div className="w-full bg-base-100 rounded-[3rem] shadow-2xl shadow-base-300/50 border border-base-300 p-8 md:p-12 overflow-hidden relative">
                <AnimatePresence mode="wait">
                    {step === 1 && (
                        <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <div className="text-center border-b border-base-200 pb-8">
                                <h2 className="text-2xl font-black text-base-content mb-2">{getConfig('supplier_welcome_shop_title', 'Informations de la Boutique')}</h2>
                                <p className="text-base-content/50 font-bold uppercase tracking-widest text-[10px]">{getConfig('supplier_welcome_shop_subtitle', 'Comment les clients vont-ils vous connaître ?')}</p>
                            </div>

                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-base-content/40 px-4">{getConfig('supplier_welcome_shop_name_label', 'Nom de la Boutique / Entreprise')}</label>
                                    <input
                                        type="text"
                                        value={formData.shopName}
                                        onChange={(e) => setFormData({ ...formData, shopName: e.target.value })}
                                        className="w-full bg-base-200 border-none rounded-3xl px-8 py-5 font-black text-sm text-base-content focus:ring-2 focus:ring-primary/20 transition-all"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-base-content/40 px-4">{getConfig('supplier_welcome_phone_label', 'Téléphone Principal')}</label>
                                        <div className="relative">
                                            <Phone className="absolute left-6 top-1/2 -translate-y-1/2 text-base-content/40" size={18} />
                                            <input
                                                type="tel"
                                                value={formData.phone}
                                                onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                                                className="w-full bg-base-200 border-none rounded-3xl pl-16 pr-8 py-5 font-black text-sm text-base-content focus:ring-2 focus:ring-primary/20 transition-all"
                                                placeholder="+229 ..."
                                            />
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-base-200 border-none rounded-3xl px-8 py-5 font-black text-sm text-base-content focus:ring-2 focus:ring-emerald-500/20 transition-all"
                                            placeholder="+229 ..."
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-amber-50 border-none rounded-3xl px-8 py-5 font-black text-sm text-base-content focus:ring-2 focus:ring-amber-500/20 transition-all"
                            ...
                            <div className="pt-4">
                                <button onClick={handleNext} disabled={!formData.shopName || !formData.phone} className="w-full py-5 bg-primary text-white rounded-3xl font-black uppercase tracking-widest text-[10px] hover:brightness-90 transition-all shadow-xl shadow-primary/20 flex items-center justify-center gap-3 disabled:opacity-50">
                    ...
                    {step === 2 && (
                        <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <div className="text-center border-b border-base-200 pb-8">
                                <h2 className="text-2xl font-black text-base-content mb-2">{getConfig('supplier_welcome_step2_title', 'Où récupérer les colis ?')}</h2>
                                <p className="text-base-content/50 font-bold uppercase tracking-widest text-[10px]">{getConfig('supplier_welcome_step2_subtitle', 'Indiquez l adresse de collecte pour nos livreurs')}</p>
                            </div>
                            ...
                            <div className="flex gap-4 pt-4">
                                <button onClick={handleBack} className="p-5 bg-base-300 text-base-content/40 rounded-3xl hover:brightness-95 transition-all">
                                    <ChevronLeft size={24} />
                                </button>
                                <button onClick={handleNext} disabled={!formData.quartier_label} className="flex-1 py-5 bg-primary text-white rounded-3xl font-black uppercase tracking-widest text-[10px] hover:brightness-90 transition-all shadow-xl shadow-primary/20 flex items-center justify-center gap-3 disabled:opacity-50">
                    ...
                    {step === 3 && (
                        <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <div className="text-center bg-base-200 rounded-[2rem] p-6 mb-8">
                                <ShieldCheck size={40} className="mx-auto text-primary mb-4" />
                                <h2 className="text-2xl font-black text-base-content mb-2">{getConfig('supplier_welcome_step3_title', 'Accord Marchand')}</h2>
                                <p className="text-base-content/50 font-bold uppercase tracking-widest text-[10px]">{getConfig('supplier_welcome_step3_subtitle', 'Signature et validation')}</p>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <label className="flex items-center gap-4 cursor-pointer p-6 rounded-3xl border-2 border-base-300 hover:border-primary/50 transition-all group">
                                    <input
                                        ...
                                        className="w-6 h-6 rounded-lg text-primary focus:ring-0 border-base-300 transition-all cursor-pointer"
                                    />
                                    <span className="text-xs font-black text-base-content/70 group-hover:text-primary transition-colors">
                            ...
                            <div className="flex gap-4 pt-6">
                                ...
                                <button
                                    ...
                                    className="flex-1 py-5 bg-neutral text-white rounded-3xl font-black uppercase tracking-widest text-[10px] hover:bg-primary transition-all shadow-xl shadow-base-300/20 disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                                    {loading ? (
