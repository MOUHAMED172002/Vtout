    ...
    return (
        <div className="min-h-screen bg-base-200 flex items-center justify-center px-4 py-12 md:py-20">
            <motion.div
                ...
                className="bg-base-100 p-8 md:p-16 rounded-[2rem] md:rounded-[3rem] border border-gray-100 shadow-2xl text-center space-y-8 md:space-y-10 max-w-xl w-full relative overflow-hidden"
            >
                ...
                <div className="grid grid-cols-1 gap-4 relative z-10">
                    {/* Order IDs List */}
                    <div className="bg-neutral text-white rounded-2xl p-6 space-y-3 text-left relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
                            <Package size={60} />
                        </div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-base-content/50">Références Commandes</p>
                        <div className="space-y-2">
                            <p className="font-black text-lg text-primary font-mono">#{orderId?.slice(0, 12).toUpperCase()}</p>
                            {order?.siblings?.map(sibling => (
                                <p key={sibling.id} className="font-black text-lg text-base-content/40 font-mono">#{sibling.id?.slice(0, 12).toUpperCase()}</p>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-base-200 p-4 rounded-2xl border border-base-200 space-y-1 text-left">
                            <p className="text-[9px] font-black uppercase tracking-widest text-base-content/40">Paiement</p>
                            <div className="flex items-center gap-2 overflow-hidden">
                                ...
                                <span className="text-xs font-bold text-base-content/80 truncate">{order?.payment_method === 'fedapay' ? 'FedaPay' : 'CASH'}</span>
                            </div>
                        </div>
                        <div className="bg-base-200 p-4 rounded-2xl border border-base-200 space-y-1 text-left">
                            <p className="text-[9px] font-black uppercase tracking-widest text-base-content/40">Total Panier</p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center relative z-10">
                    ...
                    <Link
                        to="/products-liste"
                        className="btn btn-ghost rounded-2xl px-8 md:px-10 h-14 font-black border border-gray-100 gap-2 text-sm hover:bg-base-200"
                    >
                        Boutique
                        <ArrowRight size={18} />
                    </Link>
