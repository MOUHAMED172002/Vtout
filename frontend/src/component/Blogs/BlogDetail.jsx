...
const BlogDetail = () => {
  ...
  const SITE_URL = import.meta.env.VITE_MAIN_SITE_URL || 'https://vtout.com';
  ...
  const shareOptions = [
    {
      ...
      url: () => `https://api.whatsapp.com/send?text=${encodeURIComponent((blog?.title || '') + ' — ' + articleUrl)}`,
      icon: (
  ...
  const metaDescription = blog.meta_description || blog.summary;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <Helmet>
        ...
        <meta property="og:site_name" content="Vtout Mag" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={metaTitle} />
        <meta name="twitter:description" content={metaDescription} />
        <meta name="twitter:image" content={metaImage} />
        <meta name="twitter:site" content="@vtout_benin" />
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 md:py-20">
        ...
        >
          ...
          <p className="text-xl text-slate-500 font-medium leading-relaxed italic border-l-4 border-indigo-500 pl-6">
            {blog.summary}
          </p>
        ...
        <motion.div 
          ...
          className="relative rounded-[2.5rem] overflow-hidden shadow-2xl shadow-indigo-200/50 mb-16 aspect-video"
        >
          <img src={metaImage} alt={blog.title} className="w-full h-full object-cover" />
        </motion.div>
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            {shareOptions.map((s) => (
              <a key={s.name} href={s.url()} target="_blank" rel="noopener noreferrer"
                className={`flex items-center gap-2 px-6 py-3 ${s.color} text-white rounded-2xl font-black text-sm transition-all hover:scale-105 shadow-lg`}>
                {s.icon}
                {s.name}
              </a>
            ))}
