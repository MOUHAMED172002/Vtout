...
const SupplierRegister = () => {
    ...
    const fieldClass = (err) => `w-full bg-base-200 border-2 rounded-3xl px-8 py-5 font-black text-sm text-base-content focus:ring-2 focus:ring-primary/20 transition-all ${err ? 'border-rose-400' : 'border-transparent'}`;
    const phoneWrap = (err) => `rounded-3xl border-2 ${err ? 'border-rose-400' : 'border-transparent'}`;

    return (
        <div className="min-h-screen bg-base-200 flex flex-col items-center justify-center p-6 md:p-12">
            {/* Intro header */}
            <div className="text-center mb-10 space-y-3">
                ...
                <p className="text-base-content/50 font-bold max-w-sm mx-auto">Ouvrez votre boutique et vendez à des milliers de clients partout au Bénin.</p>
                <button
            ...
            {(!isLoggingIn && (authMode === 'signUp' || isSignedIn)) && (
                <div className="w-full max-w-4xl mb-12">
                    <div className="flex justify-between items-center relative">
                        <div className="absolute top-1/2 left-0 w-full h-1 bg-base-300 -translate-y-1/2 z-0" />
                        ...
                        {steps.map(s => (
                            <div key={s.id} className="relative z-10 flex flex-col items-center gap-3">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 border-4 border-base-200 ${step >= s.id ? 'bg-primary text-white shadow-xl shadow-primary/20 scale-110' : 'bg-base-100 text-base-content/40'}`}>
                                    {step > s.id ? <Check size={20} /> : s.icon}
                                </div>
                                <span className={`text-[10px] font-black uppercase tracking-widest hidden md:block ${step >= s.id ? 'text-primary' : 'text-base-content/40'}`}>{s.title}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className={`w-full ${isLoggingIn ? 'max-w-md' : 'max-w-2xl'} bg-base-100 rounded-[40px] shadow-2xl shadow-slate-200 border border-white p-10 md:p-16 relative overflow-hidden transition-all duration-500`}>
                <AnimatePresence mode="wait">
                            ...
                            {!isSignedIn ? (
                                <div className="space-y-6">
                                    <div className="flex justify-center gap-4">
                                        <button onClick={() => setAuthMode('signUp')} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${authMode === 'signUp' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-base-200 text-base-content/40'}`}>{getConfig('supplier_register_sign_up_button', 'S\'inscrire')}</button>
                                        <button onClick={() => setAuthMode('signIn')} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${authMode === 'signIn' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-base-200 text-base-content/40'}`}>{getConfig('supplier_register_sign_in_button', 'Se Connecter')}</button>
                                    </div>
                                    <div className="text-center">
                                        <h2 className="text-3xl font-black tracking-tighter text-base-content mb-2">{authMode === 'signUp' ? getConfig('supplier_register_heading_signup', 'Devenir Fournisseur') : getConfig('supplier_register_heading_signin', 'Connexion Partenaire')}</h2>
                                        {!isLoggingIn && <p className="text-base-content/50 font-bold uppercase tracking-widest text-[10px]">{getConfig('supplier_register_step1_subtitle', 'Étape 1 : Créez votre compte d\'accès')}</p>}
                                    </div>
                                    {authMode === 'signUp' ? <SignUp /> : <SignIn />}
                                    <p className="text-center text-xs font-bold text-base-content/40 italic">
                                ...
                                <div className="space-y-8 text-center py-12">
                                    ...
                                    <div><h3 className="text-2xl font-black text-base-content">{getConfig('supplier_register_checking_title', 'Vérification en cours...')}</h3><p className="text-base-content/40 text-[11px] uppercase tracking-widest font-bold mt-2">{getConfig('supplier_register_checking_subtitle', 'Préparation de votre espace partenaire')}</p></div>
                                    <button onClick={async () => { await signOut(); window.location.reload(); }} className="text-[10px] font-black uppercase tracking-widest text-base-content/40 hover:text-rose-500 transition-colors">{getConfig('supplier_register_use_other_account_button', 'Utiliser un autre compte')}</button>
                                </div>
                            )}
                        </motion.div>
                    )}
                    ...
                    {step === 2 && (
                        <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <div className="text-center border-b border-base-200 pb-6">
                                <h2 className="text-3xl font-black tracking-tighter text-base-content mb-1">{getConfig('supplier_register_step2_title', 'Infos Partenaire')}</h2>
                                <p className="text-base-content/40 font-bold uppercase tracking-widest text-[10px]">{getConfig('supplier_register_step2_subtitle', 'Tous les champs sont obligatoires')}</p>
                            </div>

                            <div className="space-y-5">
                                {/* Shop name */}
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-base-content/40 px-4 flex items-center gap-1">{getConfig('supplier_register_shop_name_label', 'Nom de la Boutique')} <span className="text-rose-500">*</span></label>
                            ...
                            <div className="flex gap-4 pt-2">
                                <button onClick={() => setStep(s => s - 1)} className="p-5 bg-base-200 text-base-content/40 rounded-3xl hover:bg-base-300 transition-all"><ChevronLeft size={24} /></button>
                                <button onClick={() => { if (validateStep2()) setStep(s => s + 1); }} className="flex-1 py-5 bg-neutral text-white rounded-3xl font-black uppercase tracking-widest text-[10px] hover:bg-primary transition-all shadow-xl">{getConfig('supplier_register_next_button', 'Étape Suivante')}</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 3 && (
                        <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                            <div className="text-center">
                                <h2 className="text-3xl font-black tracking-tighter text-base-content mb-1">{getConfig('supplier_register_step3_title', 'Adresse de Boutique')}</h2>
                                <p className="text-base-content/40 font-bold uppercase tracking-widest text-[10px]">{getConfig('supplier_register_step3_subtitle', 'Localisation exacte pour la logistique')}</p>
                            </div>
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-base-content/40 px-4 flex items-center gap-1"><MapPin size={12} /> {getConfig('supplier_register_address_description_label', 'Description précise de l\'adresse')} <span className="text-rose-500">*</span></label>
                            ...
                            <div className="flex gap-4 pt-2">
                                <button onClick={() => setStep(s => s - 1)} className="p-5 bg-base-200 text-base-content/40 rounded-3xl hover:bg-base-300 transition-all"><ChevronLeft size={24} /></button>
                                <button onClick={() => { if (validateStep3()) setStep(s => s + 1); }} className="flex-1 py-5 bg-neutral text-white rounded-3xl font-black uppercase tracking-widest text-[10px] hover:bg-primary transition-all shadow-xl">{getConfig('supplier_register_next_button', 'Valider la position')}</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 4 && (
                        <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            ...
                            <div className="max-h-[240px] overflow-y-auto p-8 rounded-[30px] bg-base-200 text-[11px] font-bold text-base-content/70 leading-relaxed border border-base-200 shadow-inner">
                                <h4 className="font-black text-base-content text-sm mb-4 uppercase">{getConfig('supplier_register_terms_title', 'Conditions du programme Fournisseur')}</h4>
                                {policies.length > 0 ? policies.map(p => (
                                    <div key={p.id} className="mb-4"><p className="font-black text-base-content/90 text-xs mb-1">{p.title}</p><div className="whitespace-pre-wrap">{p.content}</div></div>
                                )) : (
                            ...
                            <div className="space-y-4">
                                <label className="flex items-center gap-4 cursor-pointer p-4 rounded-[30px] border-2 border-base-200 hover:border-primary transition-all group">
                                    <input type="checkbox" checked={formData.termsAccepted} onChange={e => setFormData(p => ({ ...p, termsAccepted: e.target.checked }))} className="w-8 h-8 rounded-xl border-none bg-base-200 text-primary focus:ring-0 checked:bg-primary transition-all cursor-pointer" />
                                    <span className="text-xs font-black text-base-content/70 group-hover:text-primary transition-colors">{getConfig('supplier_register_terms_accept_label', 'J\'ai lu et j\'accepte les termes du contrat ci-dessus.')}</span>
                                </label>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-base-content/40 px-4">{getConfig('supplier_register_signature_label', 'Signature Électronique')} <span className="text-rose-500">*</span></label>
                            ...
                            <div className="flex gap-4">
                                <button onClick={() => setStep(s => s - 1)} className="p-5 bg-base-200 text-base-content/40 rounded-3xl hover:bg-base-300 transition-all"><ChevronLeft size={24} /></button>
                                <button onClick={handleSubmit} disabled={!formData.termsAccepted || !formData.electronicSignature || isLoading} className="flex-1 py-5 bg-primary text-white rounded-3xl font-black uppercase tracking-widest text-[10px] hover:bg-neutral transition-all shadow-xl shadow-primary/20 disabled:opacity-50 flex justify-center items-center gap-2">
