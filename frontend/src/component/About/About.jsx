  ...
  return (
    <div className="bg-base-100 min-h-screen">
      ...
      <section className="relative pt-16 pb-10 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-base-200 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2 opacity-40"></div>

        <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
          <div className="flex flex-col gap-8 md:gap-12">
            
            {/* 1. Brand Header */}
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 border-b border-base-200 pb-8">
              <div className="space-y-6 lg:max-w-3xl">
                ...
                <h1 className="text-7xl md:text-[10rem] font-black text-base-content tracking-tighter leading-[0.85]">
                  {title.split(' ')[0]} <br />
                  <span className="text-base-content/20">{title.split(' ').slice(1).join(' ')}</span>
                </h1>
              </div>
              <div className="lg:max-w-md lg:text-right space-y-4">
                <p className="text-3xl md:text-4xl font-bold text-base-content/40 leading-tight">
                  {tagline}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
              ...
              <div className="lg:col-span-8 space-y-12">
                <div className="prose prose-slate max-w-none">
                  <p className="text-xl md:text-2xl text-base-content/50 leading-relaxed font-medium selection:bg-primary/10 whitespace-pre-wrap">
                ...
                <div className="flex flex-wrap gap-6 pt-6">
                  <button onClick={() => navigate('/products-liste')} className="group relative overflow-hidden btn btn-primary rounded-2xl px-12 h-16 font-black text-lg shadow-2xl shadow-primary/30">
                    ...
                    <div className="absolute inset-0 bg-base-100/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                  </button>
                  <button
              ...
              <div className="lg:col-span-4 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-6">
                {stats.map((stat, i) => (
                  <motion.div
                    ...
                    className="bg-base-200 p-8 rounded-[2rem] border border-base-200 flex items-center gap-6 group hover:bg-base-200 hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500"
                  >
                    <div className="w-14 h-14 bg-base-100 rounded-xl shadow-sm flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors duration-500">
                      {React.cloneElement(stat.icon, { size: 24 })}
                    </div>
                    <div>
                      <p className="text-3xl font-black text-base-content tracking-tight">{stat.value}</p>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-base-content/40 group-hover:text-base-content/70 transition-colors">{stat.label}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
      ...
      {/* Values Section */}
      <section className="py-16 bg-neutral text-white relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            <div className="space-y-4">
              <h2 className="text-4xl md:text-5xl font-black tracking-tighter text-base-content">L'Équipe <span className="text-base-content/30">derrière tout ça</span></h2>
              <p className="text-base-content/50 font-bold max-w-xl">Des passionnés qui travaillent d'arrache-pied pour rendre votre expérience inoubliable.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {team.map((member, i) => (
              <div key={i} className="group cursor-default">
                <div className="relative aspect-square rounded-[3rem] overflow-hidden bg-base-200 mb-8 border-4 border-white shadow-2xl shadow-slate-200">
                ...
                <h4 className="text-2xl font-black text-base-content tracking-tight">{member.name}</h4>
                <p className="text-base-content/40 font-black uppercase text-[10px] tracking-[0.2em]">{member.role}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="relative z-10 flex flex-col items-center text-center space-y-6">
            ...
            <div className="flex flex-wrap justify-center gap-6">
              <button
                ...
                className="btn bg-base-100 text-primary border-none rounded-2xl px-12 h-16 font-black text-xl hover:bg-base-200 shadow-xl"
              >
                Commencer mon shopping
              </button>
              <button
                onClick={() => navigate('/temoignages')}
                className="btn btn-ghost text-base-content rounded-2xl px-12 h-16 font-black text-xl border-2 border-white/30"
              >
                Voir nos avis
              </button>
            </div>
        ...
        {showFullHistory && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
            <motion.div 
              ...
              className="absolute inset-0 bg-neutral/80 backdrop-blur-xl"
            />
            <motion.div 
              initial={{ opacity: 0, scale: 0.9, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="relative bg-base-100 rounded-[3rem] w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-white/20"
            >
              <div className="p-8 md:p-12 border-b border-base-200 flex justify-between items-center bg-base-200/50">
                <div className="space-y-1">
                  <h2 className="text-3xl md:text-5xl font-black text-base-content tracking-tighter">L'Histoire de {title.split(' ')[0]}</h2>
                  <p className="text-base-content/40 font-bold uppercase text-[10px] tracking-widest">{tagline}</p>
                </div>
                <button 
                  onClick={() => setShowFullHistory(false)} 
                  className="p-4 bg-base-100 rounded-2xl hover:bg-base-200 transition-all shadow-sm border border-base-200 group"
                >
                  <X size={24} className="text-base-content/40 group-hover:text-primary transition-colors" />
                </button>
              </div>
              <div className="p-8 md:p-12 overflow-y-auto custom-scrollbar">
                <div className="prose prose-slate max-w-none">
                  <p className="text-xl md:text-2xl text-base-content/70 leading-[1.6] font-medium md:columns-2 gap-16 selection:bg-primary/10 first-letter:text-7xl first-letter:font-black first-letter:text-primary first-letter:mr-4 first-letter:float-left whitespace-pre-wrap">
                    {mission}
                  </p>
                </div>
              </div>
              <div className="p-8 border-t border-base-200 bg-base-200/30 flex justify-center">
                <button 
                  onClick={() => setShowFullHistory(false)}
