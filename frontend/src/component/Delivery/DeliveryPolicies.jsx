...
function PolicyCard({ policy, idx }) {
  ...
  return (
    <motion.div
      ...
      className="bg-base-100 rounded-[2rem] border border-base-200 shadow-sm overflow-hidden"
    >
      {/* Header — always visible */}
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between gap-4 px-7 py-6 hover:bg-base-200 transition-colors text-left"
      >
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-amber-50 text-amber-500 rounded-xl flex items-center justify-center shrink-0">
            <ShieldCheck size={20} />
          </div>
          <h3 className="text-lg font-black tracking-tight text-base-content/90">{policy.title}</h3>
        </div>
        {open
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="px-7 pb-7 space-y-5 border-t border-base-200 pt-6">
              {sections.map((sec, i) => {
                const isIntro = !sec.number && !sec.title;
                return (
                  <div key={i} className={isIntro ? '' : 'rounded-2xl border border-base-200 bg-base-200/60 p-4 space-y-2'}>
                    ...
                    {!isIntro && (
                      <div className="flex items-start gap-3">
                        ...
                        {sec.title && (
                          <h4 className="font-black text-base-content/90 text-sm leading-snug pt-0.5">{sec.title}</h4>
                        )}
                      </div>
                    )}
                    {/* Paragraphs */}
                    <div className={`space-y-2 ${!isIntro && sec.number ? 'pl-10' : ''}`}>
                      {sec.paragraphs.map((p, j) => (
                        <p key={j} className={`text-sm leading-relaxed font-medium ${isIntro ? 'text-base-content/50 italic' : 'text-base-content/70'}`}>{p}</p>
                      ))}
                    </div>
                  </div>
                );
              ...
              {/* Footer */}
              <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-base-content/40 pt-4 border-t border-base-200">
                <Clock size={11} />
...
const DeliveryPolicies = () => {
  ...
  return (
    <div className="min-h-screen bg-base-200">
      {/* Page header */}
      <div className="bg-base-100 border-b border-base-200 px-6 md:px-12 py-12">
        <div className="max-w-3xl">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.25em] text-base-content/40">Vtout · Partenaires</p>
              <h1 className="text-2xl font-black tracking-tight text-base-content leading-none mt-0.5">
                Conditions Livreurs
              </h1>
            </div>
          </div>
          <p className="text-sm text-base-content/50 font-medium max-w-xl leading-relaxed">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-10 space-y-4">
        {loading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-base-100 rounded-[2rem] border border-base-200 h-24 animate-pulse" />
          ))
        ) : policies.length === 0 ? (
          <div className="bg-base-100 rounded-[2rem] border border-base-200 py-20 flex flex-col items-center text-center gap-4">
            <div className="w-16 h-16 bg-base-200 rounded-full flex items-center justify-center">
              <ShieldCheck size={28} className="text-base-content/30" />
            </div>
            <p className="text-sm font-bold text-base-content/40">Aucun document publié pour l'instant.</p>
          </div>
        ) : (
