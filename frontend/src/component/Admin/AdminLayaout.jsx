...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex overflow-hidden">
      {/* Sidebar Desktop */}
      <aside className="hidden lg:flex flex-col w-[300px] bg-slate-900 text-white h-screen fixed left-0 top-0 shadow-2xl z-50 overflow-y-auto custom-scrollbar">
        <div className="p-10 border-b border-slate-800 flex items-center gap-3">
          <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center font-black text-xl italic shadow-lg shadow-primary/20">E</div>
          <div>
            <h1 className="text-2xl font-black tracking-tighter">EShop <span className="text-primary">Admin</span></h1>
            <p className="text-[10px] uppercase font-black tracking-widest text-slate-500">Master Control</p>
          </div>
        </div>
        ...
        <div className="p-8 border-t border-slate-800 space-y-4">
          <div className="bg-slate-800/50 p-6 rounded-3xl border border-slate-700/50 space-y-2">
            ...
            <p className="text-xs font-bold text-slate-400 leading-tight">Accès restreint aux administrateurs autorisés uniquement.</p>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 max-h-screen overflow-y-auto lg:ml-[300px]">
        ...
        {/* Header */}
        <header className="h-24 bg-white border-b border-slate-100 flex items-center justify-between px-8 md:px-12 sticky top-0 z-40 shrink-0">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="lg:hidden p-3 bg-slate-100 rounded-2xl"
            >
              <Menu size={24} />
            </button>
            <div className="hidden md:flex items-center gap-4 bg-slate-50 px-6 py-3 rounded-2xl border border-slate-100">
              <Search size={18} className="text-slate-400" />
              <input
                type="text"
                placeholder="Rechercher une commande, un client..."
                className="bg-transparent border-none text-sm font-bold text-slate-600 focus:ring-0 w-80"
              />
            </div>
          </div>
          ...
          >
            ...
            >
              <div className="mb-12 flex justify-between items-center">
                <h1 className="text-3xl font-black tracking-tighter">EShop <span className="text-primary">Admin</span></h1>
