...
export default function InventoryAlerts({ items }) {
    if (!items || items.length === 0) return (
        <div className="flex flex-col items-center justify-center p-12 bg-emerald-50/30 rounded-[2rem] border border-emerald-100/50">
            <div className="w-16 h-16 bg-base-100 rounded-full flex items-center justify-center shadow-inner mb-4">
    ...
    return (
        <div className="space-y-6">
                ...
                {items.map((item, idx) => {
                    ...
                    return (
                        <motion.div
                            ...
                            key={item.id}
                            className="group p-5 bg-base-100 rounded-2xl border border-base-200 hover:border-rose-200 hover:shadow-lg hover:shadow-rose-500/5 transition-all flex items-center gap-5"
                        >
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center relative ${item.stock === 0 ? 'bg-rose-100 text-rose-600 shadow-rose-200/50 shadow-lg' : 'bg-amber-100 text-amber-600 shadow-amber-200/50 shadow-lg'
                                ...
                                <div className="absolute -top-1 -right-1 w-5 h-5 bg-base-100 rounded-full flex items-center justify-center shadow-sm border border-base-200">
                                    <span className="text-[10px] font-black">{item.stock}</span>
                                </div>
                            </div>

                            <div className="flex-1 min-w-0">
                                <h4 className="text-sm font-black text-base-content truncate tracking-tight">{productName}</h4>
                                <p className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest truncate mt-0.5">{combination}</p>
                            </div>
            ...
            <Link
                to="/admin/products"
                className="flex items-center justify-between p-4 bg-base-200 rounded-2xl group hover:bg-primary transition-all duration-500"
            >
                <span className="text-[10px] font-black uppercase tracking-widest text-base-content/40 group-hover:text-base-content transition-colors">Gérer l'inventaire complet</span>
                <ChevronRight size={16} className="text-base-content/30 group-hover:text-base-content group-hover:translate-x-1 transition-all" />
            </Link>
        </div>
    );
}
