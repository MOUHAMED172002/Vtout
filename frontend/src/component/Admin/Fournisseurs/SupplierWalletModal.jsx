...
export default function SupplierWalletModal({ supplier, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-neutral/60 backdrop-blur-sm">
            ...
            >
                {/* Header */}
                <div className="p-8 border-b border-base-200 flex items-center justify-between bg-base-200/50">
                    <div className="flex items-center gap-5">
                        <div className="w-14 h-14 bg-emerald-100 text-emerald-600 rounded-2xl flex items-center justify-center">
                            <Wallet size={28} />
                        </div>
                        <div>
                            <h2 className="text-2xl font-black text-base-content tracking-tighter">Portefeuille Marchand</h2>
                            <p className="text-[10px] font-black uppercase text-base-content/40 tracking-widest">{supplier.name} · {supplier.phone}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="w-11 h-11 rounded-2xl bg-base-100 text-base-content/40 flex items-center justify-center hover:bg-rose-50 hover:text-rose-500 transition-all shadow-sm">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 md:p-10 space-y-10 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="p-8 bg-neutral rounded-[2rem] text-white space-y-2 relative overflow-hidden">
                            <p className="text-[10px] font-black uppercase tracking-widest text-base-content/40">Solde Disponible</p>
                            <h3 className="text-3xl font-black">{Number(data?.balance).toLocaleString()} F</h3>
                            <div className="absolute top-0 right-0 w-20 h-20 bg-base-100/5 -mr-10 -mt-10 rounded-full"></div>
                        </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                            {data?.transactions.map(t => (
                                <div key={t.id} className="p-5 bg-base-100 border border-base-200 rounded-2xl flex items-center justify-between hover:border-base-300 transition-all">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-sm font-black text-base-content/90">{t.description}</p>
                                            <div className="flex items-center gap-2 mt-1">
                                                <Calendar size={12} className="text-base-content/30" />
                                                <span className="text-[10px] font-bold text-base-content/40">{new Date(t.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest text-base-content/30">{t.status}</span>
                                    </div>
                                </div>
                            ))}

                            {data?.transactions.length === 0 && (
                                <div className="py-20 text-center bg-base-200 rounded-[2rem] border-2 border-dashed border-base-200">
                                    <History size={40} className="mx-auto text-base-content/20 mb-4" />
                                    <p className="text-xs font-black text-base-content/30 uppercase tracking-widest">Aucune transaction</p>
                                </div>
                            )}
                        </div>
                    </div>
