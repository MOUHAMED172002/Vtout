...
import {
  ...
  Upload, Star, Sparkles, Hash, Search, MapPin, Percent
} from 'lucide-react';
...
export default function AddProductModal({ onClose, onCreate, isSupplier = false, layout = 'modal' }) {
  ...
  const steps = useMemo(() => {
    ...
    if (isSupplier) {
      ...
      updatedSteps.push({ id: 'promotions', title: 'Promotions', icon: Sparkles });
    }
    return updatedSteps;
  }, [isSupplier]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const payload = {
        ...
        supplier_price: parseFloat(globalSupplierPrice) || 0,
        is_flash_sale: !isSupplier ? (data.is_flash_sale || false) : false,
    ...
    <div className={layout === 'modal' ? "fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 bg-neutral/40 backdrop-blur-sm" : ""}>
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-14 space-y-8 md:space-y-12 custom-scrollbar">
        <AnimatePresence mode="wait">
          ...
          {steps[currentStep].id === 'promotions' && (
                            ...
                            <div className="rounded-3xl border border-base-300 bg-base-100 p-4 text-xs font-black text-base-content/80">
                              {basePrice > 0 ? (
                                <>
                                  ...
                                  <p className="text-[10px] text-base-content/50">Économie totale à {tier.qty} unités : {totalSavingsAtTier.toLocaleString()} F</p>
                                </>
                              ) : (
