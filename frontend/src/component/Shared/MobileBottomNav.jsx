...
const MobileBottomNav = () => {
    ...
    return (
        ...
        >
            {/* Glass Background */}
            <div className="mx-4 mb-6 bg-base-100/80 backdrop-blur-2xl border border-white/20 shadow-2xl rounded-[2.5rem] p-2 flex items-center justify-around">
                {items.map((item, idx) => (
                    <NavLink
                        key={idx}
                        to={item.to}
                        className={({ isActive }) => 
                            `relative flex flex-col items-center gap-1 p-3 rounded-2xl transition-all duration-300 ${
                                isActive ? 'text-primary scale-110' : 'text-base-content/40'
                            }`
                        }
                    >
                        {({ isActive }) => (
