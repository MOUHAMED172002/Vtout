...
- **Approbation des produits** : Les produits des fournisseurs sont en `  En attente` par défaut. L'admin les approuve ou refuse depuis le dashboard admin (visibles via la nouvelle colonne **Statut** dans la table des produits). Le fournisseur est notifié par email automatiquement lors du changement d'état.
